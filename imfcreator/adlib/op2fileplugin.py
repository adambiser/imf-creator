...
from .instrumentfile import InstrumentFile, Instrument, Operator
...
class Op2FilePlugin(InstrumentFile):
    ...
    def load(self):
        ...
        instrument = Instrument(name=name, num_voices=2)
        ...
        self._read_voice(instrument, 0, entry[4:20])
        self._read_voice(instrument, 1, entry[20:36])
        return instrument

    def _read_voice(self, instrument, voice_number, data):
        instrument.modulator[voice_number].set_regs(**self._get_operator_regs(data[0:6]))
        instrument.feedback[voice_number] = u8(data[6])
        instrument.carrier[voice_number].set_regs(**self._get_operator_regs(data[7:13]))
        # entry[13] = unused
        instrument.note_offset[voice_number] = s16le(data[14:16])

    def _get_operator_regs(self, data):
        return {
            "tvskm": u8(data[0]),
